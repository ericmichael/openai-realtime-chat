...
"""
    ...
    # Add new tab for Knowledge Graph
    with gr.Tab("Manage Knowledge Graph"):
        ...
        def save_node(name, node_type, description, current_selection):
            ...
            try:
                if current_selection == "None":
                    node = knowledge_graph_service.create_node(
        ...
        def add_relationship(source, rel_type, target):
            ...
            try:
                ...
                with Session() as session:
                    source_node = (
                        session.query(Node).filter_by(name=source_name).first()
                    )
                    target_node = (
                        session.query(Node).filter_by(name=target_name).first()
                    )
        ...
        # Fix the save_node event handler
        save_node_btn.click(
            fn=save_node,
            inputs=[node_name, node_type_input, node_description, node_list],
            outputs=[node_list, source_node, target_node],  # Simplified outputs
        )
    ...
    with gr.Tab("Search Knowledge Graph"):
        gr.Markdown(
            "Search and explore the knowledge graph using natural language queries"
        )
        ...
        def search_knowledge_graph(query: str, max_hops: int):
            ...
            with Session() as session:
                initial_results, relationships = (
                    knowledge_graph_service.search_knowledge_graph(
                        query, max_hops, session
                    )
                )
                ...
                return (
                    gr.update(value=pd.DataFrame(initial_results), visible=True),
                    gr.update(value=pd.DataFrame(relationships), visible=True),
                )
